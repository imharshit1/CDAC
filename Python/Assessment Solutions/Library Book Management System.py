...
def load_catalog_from_file(filepath: str, catalog: list[dict]) -> list[dict]:
    """Parses books.txt line-by-line using split('|') and reconstructs dictionary list."""
    with open(filepath) as file:
        catalog = []

        for line in file:
            id, name, author, genre, price, copies = line.strip().split("|")

            book_entry = dict(
                id=int(id.strip()),
                Book_Title=name.strip(),
            ...
            catalog.append(book_entry)
        print("Old Catalog loaded Successfully")
    return catalog
    ...
    while(True):
        ...
        match action:
                ...
                case 5:
                    delete_book(catalog)
                case 6:
                    sync_catalog_to_file("Assignments/Assessment/books.txt", catalog)
                case 7:
                    catalog = load_catalog_from_file("Assignments/Assessment/books.txt", catalog)
                    if catalog:
